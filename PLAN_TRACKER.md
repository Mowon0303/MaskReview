...
| P8 | CVAT 插件雏形 | 把本地复核闭环接入真实标注工作流 | `docs/cvat_plugin.md`, `src/cvat/export.py`, `src/cvat/client.py`, `scripts/cvat_sync.py`, `tests/test_cvat_export.py` | In progress | 设计文档 + step 1 离线导出层（单测）+ step 2 活的 cvat-sdk client/CLI 均完成；已对自托管 CVAT 端到端跑通：pull task（帧+种子框）→ SAM2 → masks 经 COCO RLE 导入（blackswan task 50 帧 mask 落地）→ review_queue→issues。待补：issues 路径在 flaggy 片上的实测、export 回写后的交互统计、native mask shape（路 B）精修 | 用 flaggy 片（如 drift-straight/卧室瓶子）验 issues + OVERSIZE 落地；接评估 comparison |
