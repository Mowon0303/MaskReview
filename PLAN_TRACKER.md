...
| P8 | CVAT 插件雏形 | 把本地复核闭环接入真实标注工作流 | `docs/cvat_plugin.md`, `src/cvat/export.py`, `src/cvat/client.py`, `scripts/cvat_sync.py`, `tests/test_cvat_export.py` | In progress | 设计 + 离线导出层（单测）+ 活的 cvat-sdk client/CLI 均完成，并对自托管 CVAT **端到端验证**：(1) blackswan task 50 帧 mask 经 COCO RLE 落地、queue 干净 0 issue；(2) drift-straight 2 个 edge-contact issue 落地、2 个 damped trending 帧被 min_score 正确排除；(3) 388 帧 blob 的 366 mask + 20 个 `mask_oversized` issue 落地；(4) export 回写 COCO（50 img/ann）跑通；(5) **路 B native mask shape** 实现并实测（task1 50 个可编辑 mask shape 落地）；(6) **交互统计** wiring 实现并实测（push 写 estimate sidecar，stats 命令对比 CVAT 已解决 issue，coverage 随模拟修正 0.0→0.5）。MVP 全链路验证完成（masks 双路 / issues+OVERSIZE / damping 过滤 / export / 交互统计）。待补：真实多帧人工标注 pass 的实测、把统计接进 P7 评估 comparison | 真实标注 pass 后回归；按需接 P7 |
