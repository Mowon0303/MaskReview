...
| P8 | CVAT 插件雏形 | 把本地复核闭环接入真实标注工作流 | `docs/cvat_plugin.md`, `src/cvat/export.py`, `src/cvat/client.py`, `scripts/cvat_sync.py`, `tests/test_cvat_export.py` | In progress | 设计 + 离线导出层（单测）+ 活的 cvat-sdk client/CLI 均完成，并对自托管 CVAT **端到端验证**：(1) blackswan task 50 帧 mask 经 COCO RLE 落地、queue 干净 0 issue；(2) drift-straight 2 个 edge-contact issue 落地、2 个 damped trending 帧被 min_score 正确排除；(3) 388 帧 blob 的 366 mask + 20 个 `mask_oversized` issue 落地；(4) export 回写 COCO（50 img/ann）跑通。待补：人工修正后的交互统计对比、native mask shape（路 B）精修、把 push/export 接进评估 comparison | 收尾：交互统计接 comparison；按需做路 B |
